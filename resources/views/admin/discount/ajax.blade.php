@if(isset($ajax))
    @switch($ajax)
        ...
        <table class="table">
            <thead>
            <tr>
                <th scope="col" class="col-auto">Процент скидки</th>
            ...
            @foreach($elements as $element)
                <tr class="js-field field">
                    @include('admin.discount.ajax', ['element' => $element, 'ajax' => 'show'])
                </tr>
            @endforeach
            </tbody>
        </table>
        ...
        @case('show')
        @include('admin.discount.ajax', ['element' => $element, 'ajax' => 'item'])
        @break
            ...
            <ul class="justify-content-end list-unstyled d-flex">
                <li class="ml-3">
                    <a href="javascript:void(0);"
                       data-href="{{ route('discount.edit', [$element]) }}"
                       class="js-edit" title="Редактировать"><i class="bi bi-pencil"></i></a>
                </li>
                <li class="ml-3">
                    <a href="javascript:void(0);"
                       data-href="{{ route('discount.destroy', [$element]) }}"
        ...
        <td colspan="3" class="form_row">
            <form action="{{ route('discount.update', [$element]) }}" method="post">
                @method('PATCH')
                @csrf
                <table>
                    <tr>
