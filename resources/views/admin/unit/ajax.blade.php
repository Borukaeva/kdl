@if(isset($ajax))
    @switch($ajax)
        ...
        <table class="table">
            ...
            @foreach($elements as $element)
                <tr class="js-field field">
                    @include('admin.unit.ajax', ['element' => $element, 'ajax' => 'show'])
                </tr>
            @endforeach
            </tbody>
        </table>
        ...
        @case('show')
        @include('admin.unit.ajax', ['element' => $element, 'ajax' => 'item'])
        @break
            ...
            <ul class="justify-content-end list-unstyled d-flex">
                <li class="ml-3">
                    <a href="javascript:void(0);"
                       data-href="{{ route('unit.edit', [$element]) }}"
                       class="js-edit" title="Редактировать"><i class="bi bi-pencil"></i></a>
                </li>
                <li class="ml-3">
                    <a href="javascript:void(0);"
                       data-href="{{ route('unit.destroy', [$element]) }}"
        ...
        <td colspan="3" class="form_row">
            <form action="{{ route('unit.update', [$element]) }}" method="post">
                @method('PATCH')
                @csrf
                <table>
                    <tr>
