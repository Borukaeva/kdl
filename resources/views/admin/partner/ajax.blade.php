    ...
    @switch($ajax)
        ...
        <table class="table">
            ...
            @foreach($elements as $element)
                @include('admin.partner.ajax', ['element' => $element, 'ajax' => 'show'])
            @endforeach
            </tbody>
        </table>
        ...
        <tr class="js-field field">
            @include('admin.partner.ajax', ['element' => $element, 'ajax' => 'item'])
        </tr>
        @break
        ...
        <td>
            <ul class="justify-content-end list-unstyled d-flex">
                <li class="ml-3">
                    <a href="{{ route('partner.edit', [$element]) }}" title="Редактировать"><i
                            class="bi bi-pencil"></i></a>
                </li>
                <li class="ml-3">
                    <a href="javascript:void(0);"
                       data-href="{{ route('partner.destroy', [$element]) }}"
        ...
        @case('add')
        <form
            action="@if(isset($element)) {{ route('partner.update', ['partner' => $element]) }} @else {{ route('partner.store') }} @endif"
            method="post">
            @if(isset($element))
                @method('put')
            @endif
        ...
        <td colspan="3" class="form_row">
            <form action="{{ route('partner.update', [$element]) }}" method="post">
                @method('PATCH')
                @csrf
                <table>
                    <tr>
